...
const Home = () => {
  ...
  const handleStream = (e) => {
    ...
    console.log("calling")
    if (!peerId) {
      window.alert("Input the peerid");
    }
    if (!selfPeerId || !peer || !ready) {
      window.alert("Please wait a min and retry");
    } else {
      peer.call(peerId, window.stream);
    ...
    <div>
      ...
      <div>
        <form onSubmit={handleStream}>
          <input type="text" name="" id="" value={peerId} onChange={inputPeerId} />
